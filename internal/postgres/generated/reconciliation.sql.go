...
const getFinancialReconciliation = `-- name: GetFinancialReconciliation :one
SELECT
    (SELECT count(*)::bigint
...
type GetFinancialReconciliationRow struct {
	CompletedEventsWithoutPayment   int64 `db:"completed_events_without_payment" json:"completed_events_without_payment"`
	ConfirmedPaymentsWithoutOutcome int64 `db:"confirmed_payments_without_outcome" json:"confirmed_payments_without_outcome"`
	WalletBalanceMismatches         int64 `db:"wallet_balance_mismatches" json:"wallet_balance_mismatches"`
	ReservationCountMismatches      int64 `db:"reservation_count_mismatches" json:"reservation_count_mismatches"`
	ReviewCasesWithoutReason        int64 `db:"review_cases_without_reason" json:"review_cases_without_reason"`
}

func (q *Queries) GetFinancialReconciliation(ctx context.Context) (GetFinancialReconciliationRow, error) {
	row := q.db.QueryRow(ctx, getFinancialReconciliation)
	var i GetFinancialReconciliationRow
	err := row.Scan(
		&i.CompletedEventsWithoutPayment,
		&i.ConfirmedPaymentsWithoutOutcome,
		&i.WalletBalanceMismatches,
		&i.ReservationCountMismatches,
		&i.ReviewCasesWithoutReason,
	)
	return i, err
}
